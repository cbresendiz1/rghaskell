...
main = do unsetEnv "LIQUIDHASKELL_OPTS"
   ...
   where
     ...
     tests :: IO TestTree
     tests = group "Tests" $ -- group "Tests" $ microTests  :
             --                errorTests  :
                             macroTests  :
                             proverTests :
             --                benchTests  :
...
_impLibOrder = Nothing -- Just . mkOrder $ [ "T1102_LibZ.hs", "WrapLibCode.hs", "STLib.hs", "T1102_LibY.hs" ]

_dconPosOrder :: Maybe FileOrder
_dconPosOrder = Nothing -- Just . mkOrder $ [ "Data02Lib.hs" ]

_measPosOrder :: Maybe FileOrder
_measPosOrder = Nothing -- Just . mkOrder $ [ "List00Lib.hs" ]

proverOrder :: Maybe FileOrder
proverOrder = Nothing -- Just . mkOrder $
--   [ "Proves.hs"
--   , "Helper.hs"
--   ]

icfpOrder :: Maybe FileOrder
icfpOrder = Nothing -- Just . mkOrder $
--   [ "RIO.hs"
--   , "RIO2.hs"
--   , "WhileM.hs"
--   , "DataBase.hs"
...
vectOrder = Nothing -- Just . mkOrder $
...
textOrder = Nothing -- Just . mkOrder $ 
...
proverTests = group "Prover"
   [ testGroup "foundations"     <$> dirTests  "tests/sf"       []    ExitSuccess
